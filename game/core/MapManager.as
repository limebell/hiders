	...
	public class MapManager extends EventDispatcher {
		...
		private function moveLeftHandler(e: MapEvent): void {
			...
			if (!isBuilding(_globalLocation)) {
				...
				for (var i: int = 0; i < _globalLocation.length; i++)
					if (_globalLocation.charAt(i) == "-") break;
		...
		private function redrawFromTo(from: String = "$", to: String = "$"): void {
			var _from: String = (from == "$" ? _globalLocation : from),
				_to: String = (to == "$" ? _globalLocation : to);
			var i: int, j: int;
			...
			if (!isBuilding(_globalLocation)) {
				...
				if (Map.buildingFloor(_from) > Map.buildingFloor(_to))
					j = Map.buildingFloor(_to);
				else
					j = Map.buildingFloor(_from);

				if (j < 2) {
					for (i = 0; i < _world.map.buildingAt(Map.buildingNum(_globalLocation)).buildingWidth + 4; i++) {
						if (isFloorInRange(i, Map.buildingIndex(_from), Map.buildingIndex(_to)))
							_world.buildingFloors[Map.buildingNum(_globalLocation)][i].visible = true;
						else
							_world.buildingFloors[Map.buildingNum(_globalLocation)][i].visible = false;
					}
				}
			}
/*
			for (i = 0; i < _world.map.numObjects(); i++) {
				...
				if (isObjectInRange(i, _from, _to))
					object.clip().visible = true;
				else
					object.clip().visible = false;
			}
*/
		...
		private function isObjectInRangeCave(i: int, from: String, to: String): Boolean {
			var _small: int, _large: int;
